// AI 对弈模块 - 极小化极大算法 + Alpha-Beta剪枝（Android 离线资源）
...
export class ChessAI {
    constructor(chess) {
        this.chess = chess;
        // 平板首版优先保证界面响应；后续可迁移到 Web Worker 后再提高深度。
        this.maxDepth = 3;
    ...
    alphaBetaSearch() {
        ...
        for (let i = 0; i < sortedMoves.length; i++) {
            ...
            // 递归搜索（对手视角，所以取负值）
            let score = -this.alphaBeta(this.maxDepth - 1, -beta, -alpha);

            // 有其他走法时，主动回避回到旧局面以及连续追将。
            // 将死分通常达到 10000，获胜走法不受此惩罚。
            score -= this.calculateCyclePenalty(moveResult, sortedMoves.length, score);
    ...
    calculateCyclePenalty(moveResult, alternativeCount, score) {
        if (alternativeCount <= 1 || score >= 9000) return 0;

        const repeatedPositions = this.chess.getPositionOccurrenceCount();
        const consecutiveChecks = this.chess.countConsecutiveChecks(moveResult.currentPlayer);
        const repetitionPenalty = repeatedPositions * 2500;
        const longCheckPenalty = moveResult.isCheck && consecutiveChecks >= 3 ? 5000 : 0;
        return repetitionPenalty + longCheckPenalty;
    }
