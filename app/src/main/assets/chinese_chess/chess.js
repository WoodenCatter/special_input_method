...
export class ChineseChess {
    ...
    /** 获取不会令己方帅/将处于被攻击状态的合法移动。 */
    getLegalMoves(x, y) {
        const piece = this.getPiece(x, y);
        if (!piece) return [];

        const isRed = this.isRed(piece);
        return this.getPseudoLegalMoves(x, y).filter(move =>
            !this.wouldLeaveKingInCheck(x, y, move.x, move.y, isRed)
        );
    }

    /**
     * 获取只满足棋子本身移动方式的候选移动。
     * 此方法不检查送将，攻击判断必须使用它以避免递归。
     */
    ...
    wouldLeaveKingInCheck(fromX, fromY, toX, toY, isRed) {
        ...
        const inCheck = this.isInCheck(isRed);
    ...
    isInCheck(isRed) {
        ...
        for (let y = 0; y < 10; y++) {
            for (let x = 0; x < 9; x++) {
                ...
                if (this.getPseudoLegalMoves(x, y).some(move =>
                    move.x === king.x && move.y === king.y
                )) {
                    return true;
                }
            }
    ...
    getAllLegalMoves(isRed) {
        ...
        for (let y = 0; y < 10; y++) {
            for (let x = 0; x < 9; x++) {
                const piece = this.getPiece(x, y);
                if (!piece || this.isRed(piece) !== isRed) continue;
                for (const to of this.getLegalMoves(x, y)) {
    ...
    /** 棋子布局与行棋方共同构成局面标识。 */
    getPositionKey() {
        const boardKey = this.board
            .map(row => row.map(piece => piece || '.').join(''))
            .join('/');
        return `${boardKey}:${this.currentPlayer}`;
    }

    getPositionOccurrenceCount(key = this.getPositionKey()) {
        return this.positionHistory.reduce((count, item) => count + (item === key ? 1 : 0), 0);
    }

    /** 统计某一方最近连续多少次自己的落子都形成将军。 */
    countConsecutiveChecks(player) {
        let count = 0;
        for (let i = this.moveHistory.length - 1; i >= 0; i--) {
            const move = this.moveHistory[i];
            if (move.currentPlayer !== player) continue;
            if (!move.isCheck) break;
            count++;
        }
        return count;
    }
     ...
     * 执行移动
     */
    makeMove(fromX, fromY, toX, toY, skipEndCheck = false) {
        const piece = this.getPiece(fromX, fromY);
        if (!piece || !this.isCurrentPlayerPiece(piece)) {
            throw new Error('只能移动当前行棋方的棋子');
        }

        const legal = this.getLegalMoves(fromX, fromY).some(move => move.x === toX && move.y === toY);
        if (!legal) throw new Error('非法走法');

        const captured = this.getPiece(toX, toY);
        const previousGameOver = this.gameOver;
        const previousWinner = this.winner;
        const previousEndReason = this.endReason;
        const previousPositionHistoryLength = this.positionHistory.length;
        ...
        // 兼容直接吃将的终局，同时在真实落子后判断将死与困毙。
        if (captured && captured.toUpperCase() === 'K') {
            this.gameOver = true;
            this.winner = this.currentPlayer === 'red' ? 'black' : 'red';
            this.endReason = 'king_captured';
        } else {
            const nextIsRed = this.currentPlayer === 'red';
            isCheck = this.isInCheck(nextIsRed);
            if (!skipEndCheck && !this.hasAnyLegalMove(nextIsRed)) {
                this.gameOver = true;
                this.winner = nextIsRed ? 'black' : 'red';
                this.endReason = isCheck ? 'checkmate' : 'stalemate';
            } else {
                this.gameOver = false;
                this.winner = null;
                this.endReason = null;
        ...
        historyEntry.isCheck = isCheck;
        if (!skipEndCheck) {
            const positionKey = this.getPositionKey();
            this.positionHistory.push(positionKey);
            historyEntry.positionKey = positionKey;

            // 复杂的长将/长捉裁决留待后续；三次出现同一局面先按和棋终止，
            // 保证康复对局不会陷入必须手动重开的无限循环。
            if (!this.gameOver && this.getPositionOccurrenceCount(positionKey) >= 3) {
                this.gameOver = true;
                this.winner = null;
        ...
        return {
            ...
            currentPlayer: this.currentPlayer === 'red' ? 'black' : 'red',
            previousGameOver,
            previousWinner,
            previousEndReason,
            previousPositionHistoryLength,
    ...
    undoMove() {
        ...
        if (lastMove) {
            ...
            this.positionHistory.length = lastMove.previousPositionHistoryLength;
            moves.push(lastMove);
        ...
        if (secondLastMove) {
            ...
            this.board[from.y][from.x] = piece;
            this.board[to.y][to.x] = captured || null;
            this.positionHistory.length = secondLastMove.previousPositionHistoryLength;
    ...
    undoSingleMove(moveResult) {
        ...
        const {
            piece, captured, from, to, currentPlayer,
            previousGameOver = false,
            previousWinner = null,
            previousEndReason = null,
            previousPositionHistoryLength = this.positionHistory.length
        } = moveResult;
        ...
        this.endReason = previousEndReason;
        this.positionHistory.length = previousPositionHistoryLength;
